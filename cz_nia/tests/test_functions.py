...
class TestCallIdentity(TestCase):
    ...
    def test_error(self):
        with responses.RequestsMock() as rsps:
            rsps.add(
                responses.POST,
                "https://tnia.eidentita.cz/IPSTS/issue.svc/certificate",
    ...
    def test_responses_error(self):
        with responses.RequestsMock() as rsps:
            rsps.add(
                responses.POST, "https://tnia.eidentita.cz/IPSTS/issue.svc/certificate", body=ConnectionError("Bad")
            )
            with self.assertRaises(NiaException):
                _call_identity(SETTINGS, TRANSPORT)
...
class TestCallFederation(TestCase):
    ...
    def test_error(self):
        with responses.RequestsMock() as rsps:
            rsps.add(responses.POST, "https://tnia.eidentita.cz/FPSTS/Issue.svc", body=file_content("Err_response.xml"))
            with self.assertRaises(NiaException) as err:
    ...
    def test_xmlsec_error(self):
        with responses.RequestsMock() as rsps:
            rsps.add(
                responses.POST, "https://tnia.eidentita.cz/FPSTS/Issue.svc", body=XmlsecError("something failed...")
            )
            with self.assertRaises(NiaException):
                _call_federation(SETTINGS, TRANSPORT, fromstring(file_content("fp_token.xml")))
    ...
    def test_responses_error(self):
        with responses.RequestsMock() as rsps:
            rsps.add(responses.POST, "https://tnia.eidentita.cz/FPSTS/Issue.svc", body=ConnectionError("Bad"))
            with self.assertRaises(NiaException):
                _call_federation(SETTINGS, TRANSPORT, fromstring(file_content("fp_token.xml")))
    ...
    def test_token(self):
        with responses.RequestsMock() as rsps:
            rsps.add(
                responses.POST, "https://tnia.eidentita.cz/FPSTS/Issue.svc", body=file_content("FPSTS_response.xml")
            )
...
class TestCallSubmission(TestCase):
    ...
    def test_error(self):
        message = IdentificationMessage(
            {"first_name": "Eda", "last_name": "Tester", "birth_date": datetime.date(2000, 5, 1)}
        )
        with responses.RequestsMock() as rsps:
            rsps.add(
                responses.POST,
                "https://tnia.eidentita.cz/WS/submission/Public.svc/token",
                body=file_content("Err_response.xml"),
            )
            with self.assertRaises(NiaException) as err:
    ...
    def test_requests_error(self):
        ...
        with responses.RequestsMock() as rsps:
            rsps.add(
                responses.POST, "https://tnia.eidentita.cz/WS/submission/Public.svc/token", body=ConnectionError("Bad")
            )
            with self.assertRaises(NiaException):
                _call_submission(SETTINGS, TRANSPORT, fromstring(file_content("sub_token.xml")), message)
    ...
    def test_token(self):
        ...
        with responses.RequestsMock() as rsps:
            ...
            self.assertEqual(
                [child.text for child in parsed.getchildren()[0].getchildren()[0].getchildren()],
                [
                    "1d71ff1a-d732-4485-a8dc-ad4c42a8a739",
                    "OK",
                    None,
                    "DANIEL",
                    "KA\u0160P\xc1REK",
...
class TestGetPseudonym(TestCase):
    ...
    def test_pseudonym(self):
        user_data = {"first_name": "Eda", "last_name": "Tester", "birth_date": datetime.date(2000, 5, 1)}
        with responses.RequestsMock() as rsps:
            rsps.add(
                responses.POST,
                "https://tnia.eidentita.cz/IPSTS/issue.svc/certificate",
...
class TestWriteAuthenticator(TestCase):
    ...
    def test_write_authenticator(self):
        data = {
            "pseudonym": "1d71ff1a-d732-4485-a8dc-ad4c42a8a739",
            "identification": "vip_identification",
