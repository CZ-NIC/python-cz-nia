...
class TestSignaturePrepare(TestCase):
    ...
    def test_sign_everything(self):
        ...
        security, _, _ = _signature_prepare(
            envelope, self.key, None, None, signatures={"body": False, "everything": True, "header": []}
        )
        signature = security.find(QName(ns.DS, "Signature"))
        # Get all references
    ...
    def test_sign_empty(self):
        envelope = load_xml(ENVELOPE)
        security, _, _ = _signature_prepare(
            envelope, self.key, None, None, signatures={"body": False, "everything": False, "header": []}
        )
        signature = security.find(QName(ns.DS, "Signature"))
        # Get all references
        refs = signature.xpath("ds:SignedInfo/ds:Reference/@URI", namespaces={"ds": ns.DS})
    ...
    def test_sign_header_item(self):
        ...
        security, _, _ = _signature_prepare(
            envelope, self.key, None, None, signatures={"body": False, "everything": False, "header": sig_header}
        )
        signature = security.find(QName(ns.DS, "Signature"))
        # Get all references
        refs = signature.xpath("ds:SignedInfo/ds:Reference/@URI", namespaces={"ds": ns.DS})
...
class TestBinarySignature(TestCase):
    ...
    def test_signature_binary(self):
        ...
        ref = envelope.xpath(
            "soapenv:Header/wsse:Security/ds:Signature/ds:KeyInfo/wsse:SecurityTokenReference/wsse:Reference",
